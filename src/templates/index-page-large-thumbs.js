...
const LargeThumbsPageTemplate = ({
  ...
  return (
    <div className="homepage">
      ...
      <div key={2} className={"project-grid-large-thumbs " + (filter.length > 0 ? "filtered" : "")}>
      {sortedWorks.map((work,j) => <Thumb  filter={filter} work={work} key={j}></Thumb>)}
    </div>
  </div>)
}

LargeThumbsPageTemplate.propTypes = {
  image: PropTypes.oneOfType([PropTypes.object, PropTypes.string]),
  title: PropTypes.string,
}

const LargeThumbsPage = ({ data }) => {
  const allworks = Works()
  return (
    <Layout>
      <LargeThumbsPageTemplate
...
export default LargeThumbsPage

export const pageQuery = graphql`
  query LargeThumbsPageTemplate {
